...
## Grep結果のコンテキスト表示・UI統一（Task 0604.4 - 2026-06-04）

### パターン: マッチ表示は「行全体」でなく「マッチ中心のウィンドウ」
- grep結果で行全体を出すと、min化された1行（数千文字）が丸ごと出て破綻する（サクラエディタの弱点）
- マッチ位置を中心に固定幅（例120字）で切り出し、前後を `…` で省略する。短い行はそのまま全体表示
- 行/列(`line:col`)は実値で別途報告（エディタでジャンプ可能）。1マッチ=1行にすると同一行の複数マッチも個別に見える
- 抽出ロジック（`extractContext`）は純粋関数にして単体テストする（境界: 行頭/行末/マッチ長>幅/マルチバイト）

### ルール: 同種の結果表示は既存UIパターンに揃える
- 検索結果を最初インラインペインで作ったが、置換がモーダルだったため**モーダルに統一**（[Export][Copy][Close]も同一）
- 新機能の結果/ボタン群は、既存の同種UI（モーダル構造・ボタンの見た目/レイアウト）に最初から合わせる

### 落とし穴: `querySelector('.class')` は最初の1つだけ → 同クラスが増えると競合
- `.modal-close` を持つモーダルを増やしたら、`document.querySelector('.modal-close')` が新モーダルの×を拾い、既存モーダルの×が無効化された
- 複数インスタンスが存在し得るクラスは、**所属IDでスコープ**する（`#resultModal .modal-close`）か、要素ID直接指定にする

---

## リザルト表示=コピー=CSVの単一データ源化（Task 0604.3 - 2026-06-03）

### ルール: 「表示・コピー・エクスポート」は同一データ源から生成する
- 表示用とコピー/CSV用で別ロジックを持つと、片方だけ古く/モックのまま乖離する（本件: 表示=実データ・Export/Copy=モック）
- 共通の行データ生成関数（例 `buildResultRows()`）を1つ作り、表示(`<pre>`)・コピー・CSVは整形だけ変える

### パターン: 選択コピー前提のテキストは等幅 `<pre>` + user-select
- コピーしやすさ優先の一覧表示は、装飾HTML(カード/ピル)より `<pre>` が適切:
  `white-space:pre; overflow-x:auto; user-select:text; tab-size:4;` ＋等幅フォント
- **表示テキストとコピーテキストを完全一致**させると、ドラッグ選択コピーとボタンコピーの結果が一致して混乱がない
- 列はタブ区切り（`\t`）にすると等幅で整列し、Excel等への貼り付けも容易

### ルール: 同一UI部品への重複イベントバインドを避ける（所有者を一元化）
- 本件は `ui-controller` と `execution-controller` が同じ Export/Copy ボタンに `addEventListener` しており、クリップボード書き込みが競合していた
- ボタン/モーダル等のUI部品は「所有するコントローラ」を1つに決め、他からはバインドしない

### CSV出力の基本: RFC4180エスケープ
- セルを `"..."` で囲むだけでなく、内部の `"` を `""` に二重化する（`String(cell).replace(/"/g, '""')`）
