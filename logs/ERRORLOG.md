...
## [Task 0603.1] - 2026-06-03: ダブルクォート(")を含む置換ルールの表示切り詰め

### ❌ 症状
- 設定をLoadすると、`"`を含むFrom/To（例 `$root_commonspath . "/`、`="img/`）が入力欄で`"`の手前までしか表示されない（`$root_commonspath . ` / `=`）
- 保存されたJSON自体は正しい（`\"`でエスケープ保存される）が、Load後に編集して再保存すると`"`以降が失われる

### 🔍 原因
- `replacement-ui.js:RuleManager.createRuleElement()`が`ruleDiv.innerHTML`内で`value="${rule.from}"` / `value="${rule.to}"`と文字列補間していた
- 値に含まれる`"`がHTML属性を途中で終端させ、ブラウザのパースで`.value`が`"`手前に切り詰められる
- Save自体はDOMではなくメモリ上の`replacementRules`配列から行うため、初回保存JSONは正しく見える（バグはUI表示と「Load→編集→保存」経路で顕在化）

### ✅ 解決
- `value="..."`属性への補間を廃止し、要素生成後に`input.value = rule.from || ''`（DOMプロパティ）で設定
- DOMプロパティ代入はHTMLパースを経由しないため、`"`はもちろん`<`/`>`/`&`等も安全に保持される
